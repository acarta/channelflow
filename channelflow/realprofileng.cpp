...
RealProfileNG::RealProfileNG(const int jx, const int jz, const int Nd,
                             const int Ny, const Real Lx, const Real Lz,
                             const Real a, const Real b, const fieldstate state)
    : state_(state), jx_(jx), jz_(jz), Nd_(Nd), Ny_(Ny), Lx_(Lx), Lz_(Lz),
      a_(a), b_(b) {
  assert(Nd > 0);
  u_.reserve(Nd);
...
RealProfileNG::RealProfileNG(const vector<ChebyCoeff> u, const int jx,
                             const int jz, const Real Lx, const Real Lz)
    : state_(u[0].state()), jx_(jx), jz_(jz), Nd_(u.size()), Ny_(u[0].N()),
      Lx_(Lx), Lz_(Lz), a_(u[0].a()), b_(u[0].b()), u_(u) {
  for (vector<ChebyCoeff>::const_iterator it = u.begin(); it != u.end(); ++it)
    assert((*it).state() == state_);
}

RealProfileNG::RealProfileNG()
    : state_(Spectral), jx_(0), jz_(0), Nd_(1), Ny_(1), Lx_(0), Lz_(0), a_(-1),
      b_(1) {
  u_.reserve(Nd());
  ChebyCoeff u(Ny(), a(), b(), Spectral);
  u_.push_back(u);
...
RealProfileNG::RealProfileNG(const RealProfileNG &e)
    : state_(e.state()), jx_(e.jx()), jz_(e.jz()), Nd_(e.Nd()), Ny_(e.Ny()),
      Lx_(e.Lx()), Lz_(e.Lz()), a_(e.a()), b_(e.b()), u_(e.u_) {}
...
Real L2InnerProduct(const RealProfileNG &e1, const RealProfileNG &e2,
...
Real L2Norm2(const RealProfileNG &e, bool normalize) {
  Real norm = 0;
  for (vector<ChebyCoeff>::const_iterator d = e.u_.begin(); d != e.u_.end();
       ++d) {
...
vector<RealProfileNG> realBasisNG(const int Ny, const int kxmax,
                                  const int kzmax, const Real Lx, const Real Lz,
...
void orthonormalize(vector<RealProfileNG> &basis) {
  ...
  for (vector<RealProfileNG>::iterator it1 = basis.begin(); it1 != basis.end();
       ++it1) {
    for (vector<RealProfileNG>::iterator it2 = basis.begin(); it2 != it1;
         ++it2) {
...
struct SymmetryChecker : public function<bool(const RealProfileNG &)> {
  vector<FieldSymmetry> s;
  RealProfileNG work;
  Real tol;
  SymmetryChecker(const vector<FieldSymmetry> &ss, const Real tolerence)
      : s(ss), tol(tolerence) {
    ;
  }
  ...
  bool operator()(const RealProfileNG &e) {
    for (vector<FieldSymmetry>::const_iterator si = s.begin(); si != s.end();
         ++si) {
      work = e;
      work *= (*si);
      work -= e;
...
void selectSymmetries(vector<RealProfileNG> &basis,
                      const vector<FieldSymmetry> &s, const Real tolerance) {
...
void RealProfileNG::makeState(const fieldstate s) {
  if (s == Physical)
    ...
    cferror("RealProfileNG::makeState(fieldstate) : Recieved state which was "
            "neither Spectral nor Physical");
}
...
void RealProfileNG::makeState(const fieldstate s, const ChebyTransform &t) {
  if (s == Physical)
    ...
    cferror("RealProfileNG::makeState(const fieldstate, const ChebyTransform&) "
            ": Recieved state which was neither "
