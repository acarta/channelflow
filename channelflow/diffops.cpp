...
Real chebyNorm(const FlowField &f, bool normalize) {
  return sqrt(chebyNorm2(f, normalize));
}
...
Real chebyDist(const FlowField &f, const FlowField &g, bool normalize) {
        ...
        if (kx >= kxmin && kx <= kxmax) {
          for (lint mz = mzlocmin; mz < mzlocmax; ++mz) {
            ...
            if (kz >= 0 && kz <= kzmax) {
              const int cz = (kz > 0) ? 2 : 1;
              // 					int cz = 1; // cz = 2
              // for kz>0 to take account of kz<0 ghost modes
              // for (int kz=0; kz<=kzmax; ++kz) {
              sum += cy * cz *
                     abs2(f.cmplx(mx, ny, mz, i) - g.cmplx(mx, ny, mz, i));
            }
          }
        }
      }
...
#ifdef HAVE_MPI
  Real tmp = sum;
  // 		MPI_Allreduce(&tmp, &sum, 1, MPI_DOUBLE, MPI_SUM,
  // /*MPI_COMM_WORLD*/);
  MPI_Allreduce(&tmp, &sum, 1, MPI_DOUBLE, MPI_SUM, *u.comm_world());
#endif
  // 	cout << u.taskid() << ": " << sum << endl;
...
Real L2InnerProduct(const FlowField &f, const FlowField &g, int kxmax,
                    int kzmax, bool normalize) {
  assert(f.ystate() == Spectral);
  assert(g.ystate() == Spectral);
  assert(f.xzstate() == Spectral);
  assert(g.xzstate() == Spectral);
  ...
  if (kxmax < 0 || kxmax > f.kxmax() || kxmax > g.kxmax())
    kxmax = lesser(f.padded() ? f.kxmaxDealiased() : f.kxmax(),
                   g.padded() ? g.kxmaxDealiased() : g.kxmax());
  if (kzmax < 0 || kzmax > f.kzmax() || kzmax > g.kzmax())
    kzmax = lesser(f.padded() ? f.kzmaxDealiased() : f.kzmax(),
                   g.padded() ? g.kzmaxDealiased() : g.kzmax());
...
Real L2Norm2(const FlowField &u, int kxmax, int kzmax, bool normalize) {
  ...
  // EFFICIENCY: would be better with smarter looping, but prob not worth
  // trouble.
...
Real L2Dist(const FlowField &u, const FlowField &v, int kxmax, int kzmax,
            bool normalize) {
  return sqrt(L2Dist2(u, v, kxmax, kzmax, normalize));
}

Real L2Dist2(const FlowField &u, const FlowField &v, int kxmax, int kzmax,
             bool normalize) {
  Real sum = 0.0;
  assert(u.congruent(v));
  assert(u.xzstate() == Spectral && u.ystate() == Spectral);
...
Real L2Norm3d(const FlowField &u, bool normalize) {
  return sqrt(L2Norm2_3d(u, normalize));
}
...
void field2coeff(const vector<RealProfile> &basis, const FlowField &u,
                 cfarray<Real> &a) {
  int N = basis.size();
  if (a.N() != N)
    a.resize(N);
...
void coeff2field(const vector<RealProfile> &basis, const cfarray<Real> &a,
                 FlowField &u) {
  int N = basis.size();
  assert(a.N() == N);
  u.setToZero();
...
void field2coeff(const vector<BasisFunc> &basis, const FlowField &u,
                 cfarray<Complex> &a) {
  int N = basis.size();
  if (a.N() != N)
    a.resize(N);
...
void coeff2field(const vector<BasisFunc> &basis, const cfarray<Complex> &a,
                 FlowField &u) {
  int N = basis.size();
  assert(a.N() == N);
  u.setToZero();
...
void field2coeff(const std::vector<RealProfileNG> &basis, const FlowField &u,
                 std::vector<Real> &a) {
  a.clear();
  a.reserve(basis.size());

  for (vector<RealProfileNG>::const_iterator ei = basis.begin();
       ei != basis.end(); ++ei)
    a.push_back(L2InnerProduct(*ei, u));
}

void coeff2field(const std::vector<RealProfileNG> &basis,
                 const std::vector<Real> &a, FlowField &u) {
...
void randomProfile(ComplexChebyCoeff &u, ComplexChebyCoeff &v,
                   ComplexChebyCoeff &w, int kx, int kz, Real Lx, Real Lz,
                   Real mag, Real decay) {
  int N = u.length();
  ChebyTransform trans(N);
  u.setState(Spectral);
  ...
  if (DEBUG) {
    ...
    if (abs(u.eval_a()) + abs(u.eval_b()) > EPSILON ||
        abs(v.eval_a()) + abs(v.eval_b()) > EPSILON ||
...
void chebyProfile(ComplexChebyCoeff &u, ComplexChebyCoeff &v,
                  ComplexChebyCoeff &w, int un, int vn, int kx, int kz, Real Lx,
                  Real Lz, Real decay) {
  int N = u.length();
  ChebyTransform trans(N);
  u.setState(Spectral);
  ...
  [[maybe_unused]] const Real divNorm = L2Norm(div);
  [[maybe_unused]] const Real ubcNorm = abs(u.eval_a()) + abs(u.eval_b());
  [[maybe_unused]] const Real vbcNorm = abs(v.eval_a()) + abs(v.eval_b());
  [[maybe_unused]] const Real wbcNorm = abs(w.eval_a()) + abs(w.eval_b());
...
void assignOrrSommField(FlowField &u, FlowField &P, Real t, Real Reynolds,
                        Complex omega, const ComplexChebyCoeff &ueig,
  ...
  for (ny = u.nylocmin(); ny < u.nylocmax(); ++ny)
    for (int nx = u.nxlocmin(); nx < u.nxlocmin() + u.Nxloc(); ++nx)
      for (int nz = 0; nz < u.Nz(); ++nz)
        P(nx, ny, nz, 0) +=
            0.5 * (square(u(nx, ny, nz, 0)) + square(u(nx, ny, nz, 1)) +
                   square(u(nx, ny, nz, 2)));
  ...
  for (int i = 0; i < Nd; ++i) {
    for (int my = 0; my < My; ++my) {
      for (int mx = f.mxlocmin(); mx < f.mxlocmin() + f.Mxloc(); ++mx) {
        int kx = f.kx(mx);
        Complex cx = rot * (std::pow(2 * pi * kx / Lx, n) *
                            zero_last_mode(kx, kxmax, n));
  ...
  if (sxz == Spectral) {
...
#ifdef HAVE_MPI
    cferror("State Physical,Spectral requried. This state does not exist in "
            "MPI-version");
#endif
    int Nx = f.Nx();
    int Ny = f.Ny();
    ...
    for (int i = 0; i < Nd; ++i) {
      ...
      for (int ny = Nyb - 2; ny >= 0; --ny) {
        for (int nx = 0; nx < Nx; ++nx)
          for (int nz = 0; nz < Nz; ++nz)
            dfdy(nx, ny, nz, i) = dfdy(nx, ny + 2, nz, i) +
                                  scale * (ny + 1) * f(nx, ny + 1, nz, i);
      }
      for (int nx = 0; nx < Nx; ++nx)
        for (int nz = 0; nz < Nz; ++nz)
          dfdy(nx, 0, nz, i) *= 0.5;
    ...
    for (lint my = 0; my < My; ++my)
      for (lint mx = mxlocmin; mx < mxlocmax; ++mx)
        for (lint mz = mzlocmin; mz < mzlocmax; ++mz) {
          int kz = f.kz(mz);
          Complex cz =
              rot * (std::pow(az * kz, n) * zero_last_mode(kz, kzmax, n));
  ...
  for (int i = 0; i < Nd; ++i)
    for (lint my = 0; my < My; ++my)
      for (lint mx = mxlocmin; mx < mxlocmax; ++mx) {
        int kx = f.kx(mx);
        Complex cx =
            xrot * (std::pow(ax * kx, nx) * zero_last_mode(kx, kxmax, nx));
        for (lint mz = mzlocmin; mz < mzlocmax; ++mz) {
          int kz = f.kz(mz);
          Complex cz =
              zrot * (std::pow(az * kz, nz) * zero_last_mode(kz, kzmax, nz));
      ...
      for (lint my = 0; my < My; ++my)
        for (lint mx = mxlocmin; mx < mxlocmax; ++mx) {
          ...
          for (lint mz = mzlocmin; mz < mzlocmax; ++mz) {
            int kz = f.kz(mz);
            Complex czkz(0.0,
                         2.0 * pi * kz / Lz * zero_last_mode(kz, kzmax, 1));
            Complex fval = f.cmplx(mx, my, mz, i);
            gradf.cmplx(mx, my, mz, i3j(i, 0)) = fval * cxkx;
            gradf.cmplx(mx, my, mz, i3j(i, 2)) = fval * czkz;
          }
...
//             ddxf.resize(f.Nx(), f.Ny(), f.Nz(), 3, f.Lx(), f.Lz(), f.a(),
//             f.b(), f.cfmpi());
//         else
//             ddxf.setToZero();
...
//                     int kx = f.kx(mx);
//                     Complex
//                     cxkx(0.0, 2.0*pi*kx/Lx*zero_last_mode(kx,kxmax,1)); for
//                     (lint mz=mzlocmin; mz<mzlocmax; ++mz) {
// //                         int kz = f.kz(mz);
// //                         Complex
// czkz(0.0, 2.0*pi*kz/Lz*zero_last_mode(kz,kzmax,1));
//                         Complex fval = f.cmplx(mx,my,mz,i);
...
//             ddzf.resize(f.Nx(), f.Ny(), f.Nz(), 3, f.Lx(), f.Lz(), f.a(),
//             f.b(), f.cfmpi());
//         else
//             ddzf.setToZero();
...
/*******************************************************
// Superceded by faster version (following) with different loop ordering for
y-derivs. void curl(const FlowField& f_, FlowField& curlf) { FlowField& f =
(FlowField&) f_; assert(f.Nd() == 3); fieldstate sxz = f.xzstate(); fieldstate
sy  = f.ystate(); f.makeSpectral();
...
void curl(const FlowField &f_, FlowField &curlf) {
...
#ifdef HAVE_MPI // parallel data distributions requires different looping order
                // for optimal performance

  // Seperate loops (see serial below) do not make sense for the parallel data
  // distribution (18/05/2018, FR)
  for (int mz = mzlocmin; mz < mzlocmax; ++mz) {
    for (int mx = mxlocmin; mx < mxlocmax; ++mx) {
      ...
      for (int my = Myb - 2; my >= 0; --my)
        curlf.cmplx(mx, my, mz, 0) =
            curlf.cmplx(mx, my + 2, mz, 0) +
            (my + 1) * scale * f.cmplx(mx, my + 1, mz, 2);
      ...
      for (int my = Myb - 2; my >= 0; --my)
        curlf.cmplx(mx, my, mz, 2) =
            curlf.cmplx(mx, my + 2, mz, 2) -
            (my + 1) * scale * f.cmplx(mx, my + 1, mz, 0);
      curlf.cmplx(mx, 0, mz, 2) *= 0.5;
    }
  }
#else
  // It is fastest to do the y-derivs in sepratae loops. I've benchmarked.
  // 2008-004-09 jfg assign curlf[0] =  df[2]/dy;
  for (int mx = mxlocmin; mx < mxlocmax; ++mx) {
    for (int mz = mzlocmin; mz < mzlocmax; ++mz) {
      curlf.cmplx(mx, Myb, mz, 0) = zero;
      curlf.cmplx(mx, Myb - 1, mz, 0) = Myb * scale * f.cmplx(mx, Myb, mz, 2);
  ...
  // Assign d/dx and d/dz terms to curl. It's fastest to do these together.
  // (surprising)
  for (int my = 0; my < My; ++my)
    for (int mx = mxlocmin; mx < mxlocmax; ++mx) {
      int kx = f.kx(mx);
...
void outer(const FlowField &f_, const FlowField &g_, FlowField &fg) {
  ...
  if (!f.geomCongruent(fg) || fg.Nd() != f.Nd() * g.Nd())
    fg.resize(f.Nx(), f.Ny(), f.Nz(), f.Nd() * g.Nd(), f.Lx(), f.Lz(), f.a(),
              f.b(), f.cfmpi());
  else
    fg.setToZero();
...
void dot(const FlowField &f_, const FlowField &g_, FlowField &fdotg) {
  ...
  if (f.congruent(g)) {
    ...
    if (!f.congruent(fdotg) || fdotg.Nd() != 3)
      fdotg.resize(f.Nx(), f.Ny(), f.Nz(), 3, f.Lx(), f.Lz(), f.a(), f.b(),
                   f.cfmpi());
    else
      fdotg.setToZero();
    fdotg.setState(Physical, Physical);
    ...
    cerr << "error in dot(f, g, fdotg) : incompatible f and g. Exiting."
         << endl;
    exit(1);
  }
    ...
    for (int j = 0; j < 3; ++j) {
      ...
      for (lint my = 0; my < My; ++my)
        for (lint mx = mxlocmin; mx < mxlocmax; ++mx) {
          int kx = f.kx(mx);
          Complex cxkx(0.0,
                       2.0 * pi * f.kx(mx) / Lx * zero_last_mode(kx, kxmax, 1));
          for (lint mz = mzlocmin; mz < mzlocmax; ++mz) {
            int kz = f.kz(mz);
            Complex czkz(0.0,
                         2.0 * pi * kz / Lz * zero_last_mode(kz, kzmax, 1));
            divf.cmplx(mx, my, mz, j) += f.cmplx(mx, my, mz, i3j(0, j)) * cxkx +
    ...
    for (lint nz = 0; nz < Nz; ++nz)
      for (lint ny = nylocmin; ny < nylocmax; ++ny)
        for (lint i = 0; i < Nd; ++i) {
          const int j = (i + 1) % 3;
          const int k = (i + 2) % 3;
          fcg(nx, ny, nz, i) = f(nx, ny, nz, j) * g(nx, ny, nz, k) -
                               f(nx, ny, nz, k) * g(nx, ny, nz, j);
        }
#else
  for (lint i = 0; i < Nd; ++i) {
    const int j = (i + 1) % 3;
    ...
    for (int nz = 0; nz < Nz; ++nz)
      for (int ny = nylocmin; ny < nylocmax; ++ny) {
        for (int i = 0; i < 3; ++i)
          for (int j = 0; j < 3; ++j) {
            A[3 * i + j] =
                gradu(nx, ny, nz, i3j(i, j)) - gradu(nx, ny, nz, i3j(j, i));
            S[3 * i + j] =
                gradu(nx, ny, nz, i3j(i, j)) + gradu(nx, ny, nz, i3j(j, i));
          }
        Real q = 0;
...
void rotationalNL(const FlowField &u_, FlowField &f, FlowField &tmp,
...
void convectionNL(const FlowField &u_, FlowField &f, FlowField &tmp,
                  const fieldstate finalstate) {
  dotgrad(u_, u_, f, tmp);
}

void adjointTerms(const FlowField &u_, FlowField &u_dir_, FlowField &f,
                  FlowField &tmp, FlowField &tmpadj,
                  const fieldstate finalstate) {
  ...
  if (!u.geomCongruent(grad_v) || grad_v.Nd() < 9)
    grad_v.resize(u.Nx(), u.Ny(), u.Nz(), 9, u.Lx(), u.Lz(), u.a(), u.b(),
                  u.cfmpi());

  if (!u.geomCongruent(grad_uT) || grad_uT.Nd() < 9)
    grad_uT.resize(u.Nx(), u.Ny(), u.Nz(), 9, u.Lx(), u.Lz(), u.a(), u.b(),
                   u.cfmpi());
    ...
    for (lint nz = 0; nz < Nz; ++nz)
      for (lint ny = nylocmin; ny < nylocmax; ++ny)
        for (int i = 0; i < 3; ++i)
          for (int j = 0; j < 3; ++j) {
            f(nx, ny, nz, i) -=
                u(nx, ny, nz, j) * grad_v(nx, ny, nz, i3j(i, j));
            // index change i j -> j i for transpose of grad
            f(nx, ny, nz, i) +=
                v(nx, ny, nz, j) * grad_uT(nx, ny, nz, i3j(j, i));
          }
#else
  for (int i = 0; i < 3; ++i)
    for (int j = 0; j < 3; ++j)
...
void perturbationTermsNLin(const FlowField &u_, FlowField &u_nlin_,
                           FlowField &f, FlowField &tmp, FlowField &tmppert,
                           const fieldstate finalstate) {
  FlowField &v = (FlowField &)u_;
        ...
        for (int i = 0; i < 3; ++i)
          for (int j = 0; j < 3; ++j) {
            f(nx, ny, nz, i) +=
                u(nx, ny, nz, j) * grad_v(nx, ny, nz, i3j(i, j));
            f(nx, ny, nz, i) +=
                v(nx, ny, nz, j) * grad_u(nx, ny, nz, i3j(i, j));
            f(nx, ny, nz, i) +=
                v(nx, ny, nz, j) *
                grad_v(nx, ny, nz, i3j(i, j)); // non linear term
          }
#else
  for (int i = 0; i < 3; ++i)
    for (int j = 0; j < 3; ++j)
...
void perturbationTermsLin(const FlowField &u_, FlowField &u_nlin_, FlowField &f,
                          FlowField &tmp, FlowField &tmppert,
                          const fieldstate finalstate) {
  FlowField &v = (FlowField &)u_;
  FlowField &u = (FlowField &)u_nlin_;
  FlowField &grad_v = tmp;
      ...
      for (lint ny = nylocmin; ny < nylocmax; ++ny)
        for (int i = 0; i < 3; ++i)
          for (int j = 0; j < 3; ++j) {
            ...
            f(nx, ny, nz, i) +=
                v(nx, ny, nz, j) * grad_u(nx, ny, nz, i3j(i, j));
            // f(nx,ny,nz,i) += v(nx,ny,nz,j)*grad_v(nx,ny,nz,i3j(i,j)); // non
            // linear term
          }
#else
  for (int i = 0; i < 3; ++i)
...
void divergenceNL(const FlowField &u_, FlowField &f, FlowField &tmp,
                  const fieldstate finalstate) {
  FlowField &u = (FlowField &)u_;
  FlowField &uu = tmp;
...
void skewsymmetricNL(const FlowField &u_, FlowField &f, FlowField &tmp,
                     const fieldstate finalstate) {
  FlowField &u = (FlowField &)u_;
  ...
  for (lint mx = mxlocmin; mx < mxlocmax; ++mx) {
    ...
    for (lint mz = mzlocmin; mz < mzlocmax; ++mz) {
      ...
      for (int i = 0; i < 3; ++i) {
        ...
        for (lint my = 0; my < My; ++my) {
          f.cmplx(mx, my, mz, i)
              // Add in u_j du_i/dx and u_j du_i/dz, that is, d/dx_j (u_i u_j)
              // for j=0,2
              += 0.5 * (Dx * uu.cmplx(mx, my, mz, i0) +
                        Dz * uu.cmplx(mx, my, mz, i2)) +
                 0.5 * tmpProfile_y[my]; // Add in du_i/dy, that is d/dx_j (u_i
                                         // v_j) for j=1
        }
      }
    }
    ...
    for (lint my = 0; my < My; ++my)
      for (lint mx = mxlocmin; mx < mxlocmax; ++mx) {
        ...
        for (lint mz = mzlocmin; mz < mzlocmax; ++mz) {
          int kz = u.kz(mz);
          Complex Dz(0.0, 2 * pi * kz / Lz * zero_last_mode(kz, kzmax, 1));
          f.cmplx(mx, my, mz, i) += 0.5 * (Dx * uu.cmplx(mx, my, mz, i0) +
                                           Dz * uu.cmplx(mx, my, mz, i2));
        }
      }
...
void linearizedNL(const FlowField &u_, const ChebyCoeff &U_,
                  const ChebyCoeff &W_, FlowField &f,
                  const fieldstate finalstate) {
  FlowField &u = (FlowField &)u_;
  ChebyCoeff &U = (ChebyCoeff &)U_;
...
void linearAboutFieldNL(const FlowField &u, const FlowField &ubase,
                        ...
                        FlowField &ftmp, const fieldstate finalstate) {
...
void linearizedNL(const FlowField &u_, const FlowField &ubase,
                  const ChebyCoeff &Ubase, FlowField &f, FlowField &tmp,
                  const fieldstate finalstate) {
  /*****************************************
   // Gives L2Norm(du/dt)  == 5.3628325247665347e-06 for T=1
...
void dotgrad(const FlowField &u_, const FlowField &v_, FlowField &f,
             FlowField &tmp) {
  FlowField &u = (FlowField &)u_;
...
void dotgradScalar(const FlowField &u_, const FlowField &s_, FlowField &f,
                   FlowField &tmp) {
  FlowField &u = (FlowField &)u_;
  ...
  if (!u.geomCongruent(f) || f.Nd() != 1)
    f.resize(u.Nx(), u.Ny(), u.Nz(), 1, u.Lx(), u.Lz(), u.a(), u.b(),
             u.cfmpi());
  else
    f.setToZero();

  if (!u.geomCongruent(grad_s) || grad_s.Nd() < 3)
    grad_s.resize(u.Nx(), u.Ny(), u.Nz(), 3, u.Lx(), u.Lz(), u.a(), u.b(),
                  u.cfmpi());
...
Real L2InnerProduct(const RealProfileNG &e, const FlowField &f,
                    bool normalize) {
...
FlowField extractRolls(const FlowField &u) {
  ...
  FlowField rolls(u.Nx(), u.Ny(), u.Nz(), u.Nd(), u.Lx(), u.Lz(), u.a(), u.b(),
                  u.cfmpi());
  rolls.makeSpectral();
  rolls.setToZero();
  const lint mxlocmin = u.mxlocmin();
...
Real L2Norm_uvw(const FlowField &u, const bool ux, const bool uy,
                const bool uz) {
  assert(u.ystate() == Spectral);
  assert(u.xzstate() == Spectral);
  Real sum = 0.0;
...
Real Ecf(const FlowField &u) {
  return pow(L2Norm_uvw(u, false, true, true), 2);
}

string fieldstatsheader() {
  stringstream header;
  header << setw(14) << "L2" << setw(14) << "u2" << setw(14) << "v2" << setw(14)
         << "w2" << setw(14) << "e3d" << setw(14) << "ecf" << setw(14)
         << "ubulk" << setw(14) << "wbulk" << setw(14) << "wallshear"
         << setw(14) << "wallshear_a" << setw(14) << "wallshear_b" << setw(14)
         << "dissipation";
  return header.str();
}
...
string fieldstats(const FlowField &u) {
  ...
  s << setw(14) << L2Norm(u) << setw(14) << L2Norm_uvw(u, true, false, false)
    << setw(14) << L2Norm_uvw(u, false, true, false) << setw(14)
    << L2Norm_uvw(u, false, false, true) << setw(14) << L2Norm3d(u) << setw(14)
    << Ecf(u) << setw(14) << getUbulk(u) << setw(14) << getWbulk(u) << setw(14)
    << wallshear(u) << setw(14) << wallshearLower(u) << setw(14)
    << -1 * wallshearUpper(u) << setw(14) << dissipation(u);
  return s.str();
}
