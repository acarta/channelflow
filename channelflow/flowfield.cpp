...
FlowField::FlowField(int Nx, int Ny, int Nz, int Nd, Real Lx, Real Lz, Real a,
                     Real b, CfMPI *cfmpi, fieldstate xzstate,
...
FlowField::FlowField(int Nx, int Ny, int Nz, int Nd, int tensorOrder, Real Lx,
                     Real Lz, Real a, Real b, CfMPI *cfmpi, fieldstate xzstate,
                     fieldstate ystate, uint fftw_flags)
...
FlowField::FlowField(const FlowField &f)
    : padded_(f.padded_), cfmpi_(f.cfmpi_) {
...
FlowField::FlowField(const string &filebase, CfMPI *cfmpi) {
  ...
  if (HAVE_NETCDF && isReadable(ncname))
    ...
    stringstream serr;
    serr << "error in FlowField(string filebase) : can't open filebase=="
         << filebase << endl;
    if (HAVE_HDF5)
      serr << "neither " << h5name << " nor " << ffname << " is readable"
           << endl;
    else
  ...
  // file should be readable. Further, if it has .h5, the HDF5 libs are
  // installed.
  ...
  if (hasSuffix(filename, ".nc")) {
    ...
    cferror(
        "error in FlowField(string filebase). This line should be unreachable");
#else
    using namespace H5;
    H5File h5file;
    ...
    if (taskid() == 0) {
      ...
      if (Ny != Ny_)
        cferror(
            "error in FlowField(string h5filename) : Ny != Nypad in h5 file");
    }
    ...
    if (Nx == Nx_ && Ny == Ny_ &&
        Nz == Nz_) { // means that FlowField isn't padded
#ifdef HAVE_MPI
      CfMPI_single *CfMPI_one = &CfMPI_single::getInstance();
      FlowField v(Nx, Ny, Nz, Nd_, Lx_, Lz_, a_, b_, CfMPI_one, Physical,
                  Physical);
      ...
      this->interpolate(v);
      this->setPadded(((2 * Nx_) / 3 >= Nx && (2 * Nz_) / 3 >= Nz) ? true
                                                                   : false);
    }
#endif // HAVE_HDF5_LIB
  } else {
    ...
    if (taskid() == 0) {
      ...
      if (padded_ == true && xzstate_ == Spectral) {
        ...
        // In innermost loop, cfarray index is (nz + Nzpad2_*(nx + Nx_*(ny +
        // Ny_*i))), which is the same as the FlowField::flatten function.
        for (int i = 0; i < Nd_; ++i) {
...
FlowField::FlowField(const string &filebase, int major, int minor, int update) {
  ...
  if (!is) {
    cferror("FlowField::FlowField(filebase, major,minor,update) : can't open "
            "file " +
            filename + " or " + filebase);
  }

  if (major > 0 || minor > 9) {
    stringstream serr;
    serr << "FlowField::FlowField(filebase, major,minor,update) :\n"
         << major << '.' << minor << '.' << update << " is higher than 0.9.x"
         << endl;
    cferror(serr.str());
  }
  ...
  if (update >= 16) {
    ...
    if (padded_ == true && xzstate_ == Spectral) {
      ...
      // In innermost loop, cfarray index is (nz + Nzpad2_*(nx +
      // Nx_*(ny+Ny_*i))) which is the same as the FlowField::flatten function.
      for (int i = 0; i < Nd_; ++i) {
        for (int ny = 0; ny < Ny_; ++ny) {
          for (int nx = 0; nx <= Nxd; ++nx) {
            for (int nz = 0; nz <= Nzd; ++nz)
...
void FlowField::reconfig(const FlowField &u, uint fftw_flags) {
  resize(u.Nx(), u.Ny(), u.Nz(), u.Nd(), u.Lx(), u.Lz(), u.a(), u.b(),
         u.cfmpi(), fftw_flags);
}

void FlowField::resize(int Nx, int Ny, int Nz, int Nd, Real Lx, Real Lz, Real a,
                       Real b, CfMPI *cfmpi, uint fftw_flags) {
  assert(Nx >= 0);
  assert(Ny >= 0);
  assert(Nz >= 0);
  assert(Nd >= 0);
  ...
  if (Nx == Nx_ && Ny == Ny_ && Nz == Nz_ && Nd == Nd_ && Lx == Lx_ &&
      Lz == Lz_ && a == a_ && b == b_ && cfmpi == cfmpi_ && rdata_)
    return;
  ...
  // The cfarrays are padded in  y to facilitate transpose operations =>
  // nylocmax <= nylocmin + Nylocpad Number of y-gridpoints to allocate (padded
  // to the same for each process to facilitate transposing)
  Nylocpad_ = (Ny_ % nproc0 == 0) ? Ny_ / nproc0 : Ny_ / nproc0 + 1;
  nylocmin_ = this->key0() * Nylocpad_; // Lowest ny on this process
  // nylocmin_ cannot be larger than physical Ny_ (above calculation can yield
  // this in case of large padding)
  ...
  Nloc_ = 2 * fftw_mpi_local_size_many_transposed(
                  2, rank_0, howmany, FFTW_MPI_DEFAULT_BLOCK,
                  FFTW_MPI_DEFAULT_BLOCK, cfmpi_->comm1, &Nxloc_, &nxlocmin_,
                  &Mzloc_, &mzlocmin_);

  lint rank_2[2] = {Nx_, Nypad_};
  lint alloc_local1 = fftw_mpi_local_size_many_transposed(
                          2, rank_2, 2 * Nd_, Mxloc_, Nylocpad_, cfmpi_->comm0,
                          &Mxloc_, &mxlocmin_, &Nylocpad_, &nylocmin_) /
                      2;

  // We transform cfarrays that are part of one greater contiguous cfarray --
  // verify that fftw doesn't want more memory than the absolute minimum,
  // otherwise it would silently corrupt the flowfield data
  if (taskid() < numtasks()) {
    if (alloc_local1 != Nx_ * howmany)
      cferror("FFTW tries to allocate too large cfarrays for in-place "
              "transpose. Another transpose is not "
              "implemented currently!\nThis probably means something is wrong "
              "with the MPI communicators.");
  } else {
    Mxloc_ = Mzloc_ = Nxloc_ = Nyloc_ = 0;
  }

  // FFTW uses ptrdiff_t variables for indices
  // Declare int variables to be transmitted by MPI -- should be sufficient for
  // Mx, Mz
  int tmp1 = Mxloc_;
  int tmp2 = 0;
...
void FlowField::fftw_initialize(uint fftw_flags) {
  ...
  [[maybe_unused]] static int count2 = 0;
  count++;
#ifndef HAVE_MPI
  ...
  if (Nx_ != 0 && Nz_ != 0) {
    ...
    lint howmany = Nylocpad_ * Nd_;
    xz_plan_.reset(fftw_mpi_plan_many_dft_r2c(
        2, rank_1, howmany, FFTW_MPI_DEFAULT_BLOCK, FFTW_MPI_DEFAULT_BLOCK,
        rdata_, fcdata, cfmpi_->comm1, fftw_flags | FFTW_MPI_TRANSPOSED_OUT));
    xz_iplan_.reset(fftw_mpi_plan_many_dft_c2r(
        2, rank_1, howmany, FFTW_MPI_DEFAULT_BLOCK, FFTW_MPI_DEFAULT_BLOCK,
        fcdata, rdata_, cfmpi_->comm1, fftw_flags | FFTW_MPI_TRANSPOSED_IN));
    // The transpose plans work on one slice in z at a time. Block distribution
    // changes from x to y, without changing the layout in memory (which is z x
    // y i) howmany = 2*Nd is the number of real values that is stored for the
    // last index i
    t_plan_.reset(fftw_mpi_plan_many_transpose(
        Nypad_, Mx_, 2 * Nd_, Nylocpad_, Mxlocmax_, rdata_, rdata_,
        cfmpi_->comm0, fftw_flags | FFTW_MPI_TRANSPOSED_IN));
    t_iplan_.reset(fftw_mpi_plan_many_transpose(
        Mx_, Nypad_, 2 * Nd_, Mxlocmax_, Nylocpad_, rdata_, rdata_,
        cfmpi_->comm0, fftw_flags | FFTW_MPI_TRANSPOSED_OUT));

    // The y plan operates on Nd_*2 real cfarrays at a time and is called
    // Mxloc*Mzloc times
    int ranky[1];
    ranky[0] = Ny_;
    int rankembed[1];
    ...
    // Initialize y plan
    if (Ny_ >= 2) {
      y_plan_.reset(fftw_plan_many_r2r(1, ranky, yhowmany, rdata_, rankembed,
                                       ystride, ydist, rdata_, rankembed,
    ...
    xz_plan_.reset(fftw_plan_many_dft_r2c(
        rank, real_n, howmany, rdata_, real_embed, real_stride, real_dist,
        fcdata, cplx_embed, cplx_stride, cplx_dist, fftw_flags));

    xz_iplan_.reset(fftw_plan_many_dft_c2r(
        rank, real_n, howmany, fcdata, cplx_embed, cplx_stride, cplx_dist,
    ...
    // Initialize y plan
    if (Ny_ >= 2)
      y_plan_.reset(fftw_plan_r2r_1d(Ny_, scratch_.get(), scratch_.get(),
                                     FFTW_REDFT00, fftw_flags));
    else
      y_plan_ = nullptr;
...
void FlowField::interpolate(FlowField f) {
  ...
  if (f.Lx() != g.Lx() || f.Lz() != g.Lz() || f.a() != g.a() ||
      f.b() != g.b() || f.Nd() != g.Nd()) {
    cferror("FlowField::interpolate(const FlowField& f) error:\nFlowField "
            "doesn't match argument f geometrically.\n");
  }
      ...
      for (int ny = 0; ny < fNy; ++ny)
        for (int kx = kxmin; kx < kxmax; ++kx) {
          ...
          for (int kz = 0; kz <= kzmax; ++kz) {
            ...
            //                         g.cmplx(gmx,ny,gmz,i) =
            //                         f.cmplx(fmx,ny,fmz,i);
          }
        }
    }
...
void transmit_coeff(FlowField &g, int gmx, int gmz, const FlowField &f, int fmx,
                    int fmz, int ny, int i, int taskid) {
#ifdef HAVE_MPI
  assert(f.taskid_world() ==
         g.taskid_world()); // That would mean that f and g use different
                            // communicators ...
  if (taskid == -1)         // default
    taskid = f.taskid();
  ...
  if (taskidg == taskidf && taskid == taskidf)
    ...
    if (taskid == taskidg) {
      Complex gval = 0;
      MPI_Status status;
      MPI_Recv(&gval, 1, MPI_DOUBLE_COMPLEX, taskidf, tag, *f.comm_world(),
               &status);
...
void transmit_coeff(FlowField &g, int gmx, int gmz, int ny, int i,
                    const Complex &val, int taskid_val, int taskid) {
#ifdef HAVE_MPI
  if (taskid == -1) // default
    taskid = g.taskid();
  int taskidg = g.task_coeff(gmx, gmz);
  ...
  if (taskidg == taskid_val && taskid == taskid_val)
    ...
    if (taskid == taskidg) {
      Complex gval = 0;
      MPI_Status status;
      MPI_Recv(&gval, 1, MPI_DOUBLE_COMPLEX, taskid_val, tag, *g.comm_world(),
               &status);
      g.cmplx(gmx, ny, gmz, i) = gval;
    }
  }
...
void FlowField::toprocess0(FlowField &v) const {
  ...
  MPI_Reduce(&Nloc, &bufsize, 1, MPI_UNSIGNED_LONG, MPI_MAX, 0,
             cfmpi_->comm_world);
  ...
  if (taskid() == 0) {
    ...
    for (int commtaskid = 0; commtaskid < numtasks(); commtaskid++) {
      if (commtaskid == 0) {
        ...
        MPI_Status status;
        MPI_Recv(&mxlocmin, 1, MPI_INT, commtaskid, 0, cfmpi_->comm_world,
                 &status);
        MPI_Recv(&Mxloc, 1, MPI_INT, commtaskid, 1, cfmpi_->comm_world,
                 &status);
        MPI_Recv(&mzlocmin, 1, MPI_INT, commtaskid, 2, cfmpi_->comm_world,
                 &status);
        MPI_Recv(&Mzloc, 1, MPI_INT, commtaskid, 3, cfmpi_->comm_world,
                 &status);
        MPI_Recv(&Nloc, 1, MPI_UNSIGNED_LONG, commtaskid, 4, cfmpi_->comm_world,
                 &status);
        // receive data
        MPI_Recv(recvbuf.data(), Nloc, MPI_DOUBLE, commtaskid, 5,
                 cfmpi_->comm_world, &status);
        for (int mx = mxlocmin; mx < mxlocmin + Mxloc; mx++) {
          for (int mz = mzlocmin; mz < mzlocmin + Mzloc; mz++) {
            for (int my = 0; my < Ny; my++) {
              for (int i = 0; i < Nd; i++) {
                lint index =
                    2 * (i + Nd * (my + Ny * ((mx - mxlocmin) +
                                              Mxloc * (mz - mzlocmin))));
                v.cmplx(mx, my, mz, i) =
                    Complex(recvbuf[index], recvbuf[index + 1]);
              }
            }
          }
        }
    ...
    for (int mx = mxlocmin; mx < mxlocmin + Mxloc; mx++) {
      for (int mz = mzlocmin; mz < mzlocmin + Mzloc; mz++) {
        for (int my = 0; my < Ny; my++) {
          for (int i = 0; i < Nd; i++) {
            lint index = 2 * (i + Nd * (my + Ny * ((mx - mxlocmin) +
                                                   Mxloc * (mz - mzlocmin))));
  ...
  if (taskid() == 0) {
    ...
    for (int commtaskid = 0; commtaskid < numtasks(); commtaskid++) {
      if (commtaskid == 0) {
        ...
        // send rdata
        MPI_Ssend(sendbuf.data(), Nloc, MPI_DOUBLE, commtaskid, 5,
                  cfmpi_->comm_world);
      }
    }
  } else {
    ...
    // receive data
    MPI_Status status;
    MPI_Recv(recvbuf.data(), Nloc, MPI_DOUBLE, 0, 5, cfmpi_->comm_world,
             &status);
    for (int mx = mxlocmin; mx < mxlocmin + Mxloc; mx++) {
      for (int mz = mzlocmin; mz < mzlocmin + Mzloc; mz++) {
        for (int my = 0; my < Ny; my++) {
...
ComplexChebyCoeff FlowField::profile(int mx, int mz, int i) const {
  ...
  if (xzstate_ == Spectral) {
    ...
    CfMPI_single *CfMPI_one = &CfMPI_single::getInstance();
    FlowField v(Nx_, Ny_, Nz_, Nd_, Lx_, Lz_, a_, b_,
                CfMPI_one); // FlowField v is only on process 0
    v.interpolate(*this);
    if (taskid() == 0) {
      Real vmxnymzi;
      for (int ny = 0; ny < Ny_; ++ny) {
...
int sign_i(const FieldSymmetry &sigma, int n, int Nd) {
  ...
  default:
    cferror("error : symmetry operations defined only for 3d scalar, vector, "
            "tensor fields");
    return 0;
  }
}
...
FlowField &FlowField::project(const FieldSymmetry &sigma) {
  ...
  // (u,v,w)(x,y,z) -> (sa sx u, sa sy v, sa sz w) (sx x + fx*Lx, sy y, sz z +
  // fz*Lz)
  const int s = sigma.s();
  ...
  // We have to make a choice between many (and hence possibly slow)
  // communication operations or few communcation operations but larger storage
  // requirements In this choice, we choose to communciate all data for one kx
  // at a time, which is in between communicating every (kx,ky,kz,i) separately
  // and one large cfarray for all kx
  ...
  if (sx + sz == 0) {
    ...
    for (lint ix = 0; ix < Mx_; ix++) {
      ...
      // FIXME: improve this ordering so that no process is idly waiting while
      // others communicate But be careful: it is important, that +kx and -kx
      // directly follow each other, otherwise the data might end up at the
      // wrong process
      lint kx_p = ix / 2 - ix * (ix % 2); // kx = 0,-1,1,-2,2,-3,3...
      if (kx_p >= Kxmin && kx_p <= Kxmax && mx(kx_p) >= mxlocmin_ &&
          mx(kx_p) < mxlocmin_ + Mxloc_) {
        lint mx_p = mx(kx_p);
        lint kx_m = -kx_p;
        lint mx_m = mx(kx_m);
        ...
        // Transmit -kx data
        // This is necessary because fftw aligns Mz and if the number of
        // processors in Mz is too large, some might remain empty
        if (Mzloc_ > 0) {
          if (taskid() < commtaskid) {
            MPI_Status status;
            MPI_Ssend(sendbuf.data(), bufsize, MPI_DOUBLE, commtaskid, 0,
                      MPI_COMM_WORLD);
            MPI_Recv(conjdata.data(), bufsize, MPI_DOUBLE, commtaskid, 1,
                     MPI_COMM_WORLD, &status);
          } else if (taskid() > commtaskid) {
            MPI_Status status;
            MPI_Recv(conjdata.data(), bufsize, MPI_DOUBLE, commtaskid, 0,
                     MPI_COMM_WORLD, &status);
            MPI_Ssend(sendbuf.data(), bufsize, MPI_DOUBLE, commtaskid, 1,
                      MPI_COMM_WORLD);
          }
        }
#endif
          ...
          for (int ky = 0; ky < Ny_; ++ky) {
            ...
            for (int kz = 0; kz <= Kzmax; ++kz) {
              ...
              if (mz >= mzlocmin_ && mz < mzlocmin_ + Mzloc_) {
                ...
                if (taskid() == commtaskid &&
                    kx_p >= 0) { // only kx_p >= 0: don't do things twice
                  Complex cxyz_p =
                      symmsign *
                      exp(Complex(0.0, cx * sx * kx_p + cz * sz * kz));
                  Complex cxyz_m =
                      symmsign *
                      exp(Complex(0.0, cx * sx * kx_m + cz * sz * kz));
                  if (sx == -1) {
                    ...
                    // u_{ijkl}     -> (       u_{ijkl}   + cxyz_p
                    // u^*_{i,-j,k,l})/2 u_{i,-j,k,l} -> (cxyz_m u^*_{ijkl} +
                    // u_{i,-j,k,l})/2
                    ...
                    cmplx(mx_p, ky, mz, i) =
                        0.5 * (tmp_p + cxyz_p * conj(tmp_m));
                    cmplx(mx_m, ky, mz, i) =
                        0.5 * (cxyz_m * conj(tmp_p) + tmp_m);
                  }
                } else if (taskid() != commtaskid) {
                  Complex cxyz_p =
                      symmsign *
        ...
        for (int kx = Kxmin; kx <= Kxmax; ++kx) {
          ...
          if (mx >= mxlocmin_ && mx < mxlocmin_ + Mxloc_) {
            for (int kz = Kzmin; kz <= Kzmax; ++kz) {
              ...
              if (mz >= mzlocmin_ && mz < mzlocmin_ + Mzloc_) {
                Complex cxyz =
                    symmsign * exp(Complex(0.0, cx * sx * kx + cz * sz * kz));

                if (sx == 1)
                  cmplx(mx, ky, mz, i) *= 0.5 * (1.0 + cxyz);
                else
                  cmplx(mx, ky, mz, i) =
                      0.5 * (cmplx(mx, ky, mz, i) +
                             cxyz * conj(cmplx(mx, ky, mz, i)));
              }
            }
          }
        }
...
FlowField &FlowField::operator*=(const FieldSymmetry &sigma) {
  ...
  // requirements In this choice, we choose to communciate all data for one kx
  // at a time, which is in between communicating every (kx,ky,kz,i) separately
  // and one large array for all kx
  lint bufsize = Nd_ * Ny_ * Mz_ * 2;
#ifdef HAVE_MPI
  auto sendbuf = vector<Real>(bufsize, 0.0);
#endif
              ...
              if (mz >= mzlocmin_ && mz < mzlocmin_ + Mzloc_) {
                ...
                if (taskid() == commtaskid &&
                  ...
                  if (sx == -1) {
                    ...
                    tmp = cmplx(mx_p, ky, mz, i);
                    cmplx(mx_p, ky, mz, i) =
                        cxyz_p * conj(cmplx(mx_m, ky, mz, i));
...
bool FlowField::geomCongruent(const FlowField &v, Real eps) const {
  return ((Nx_ == v.Nx_) && (Ny_ == v.Ny_) && (Nz_ == v.Nz_) &&
...
bool FlowField::congruent(const FlowField &v, Real eps) const {
  return ((Nx_ == v.Nx_) && (Ny_ == v.Ny_) && (Nz_ == v.Nz_) &&
          (Nd_ == v.Nd_) && (abs(Lx_ - v.Lx_) / Greater(Lx_, 1.0) < eps) &&
          (abs(Lz_ - v.Lz_) / Greater(Lz_, 1.0) < eps) &&
          (abs(a_ - v.a_) / Greater(abs(a_), 1.0) < eps) &&
          (abs(b_ - v.b_) / Greater(abs(b_), 1.0) < eps) &&
...
FlowField operator*(const Real a, const FlowField &w) {
  return FlowField(w) *= a;
}

FlowField operator+(const FlowField &w, const FlowField &v) {
  return FlowField(w) += v;
}
  ...
  for (int my = 0; my < My(); ++my) {
    ...
    for (int mx = 0; mx < Mx(); ++mx) {
      ...
      for (int mz = 1; mz < Mz(); ++mz) {
        ...
        Real cz = mz * gamma_z;
        u_my +=
            2 * Re(this->cmplx(mx, my, mz, i) * exp(Complex(0.0, (cx + cz))));
      }
    }
    uprof[my] = u_my;
  }
...
void FlowField::makeSpectral_y() {
...
#ifdef HAVE_MPI
  Real nrm =
      1.0 / (Ny_ - 1); // needed because FFTW does unnormalized transforms
  for (lint mz = 0; mz < Mzloc_; ++mz)
    for (lint mx = 0; mx < Mxloc_; ++mx) {
      ...
      for (lint i = 0; i < Nd_; i++) {
        ...
        cdata_[complex_flatten(mx + mxlocmin_, Ny_ - 1, mz + mzlocmin_, i)] *=
            0.5;
      }
    }
  for (lint i = 0; i < Nloc_; ++i)
  ...
  Real nrm =
      1.0 / (Ny_ - 1); // needed because FFTW does unnormalized transforms
  for (int i = 0; i < Nd_; ++i) {
    unique_ptr<Real, void (*)(void *)> scratch_handle(
        static_cast<Real *>(fftw_malloc(Ny_ * sizeof(Real))), fftw_free);
...
#ifdef HAVE_MPI
    cferror("The state Physical Spectral is not possible with MPI "
            "data-distribution");
#endif
    makeSpectral_y();
    makePhysical_xz();
...
Complex FlowField::Dx(int mx, int n) const {
  ...
  int kx_ = kx(mx);
  return rot *
         (std::pow(2 * pi * kx_ / Lx_, n) * zero_last_mode(kx_, kxmax(), n));
}
Complex FlowField::Dz(int mz, int n) const {
  ...
  int kz_ = kz(mz);
  return rot *
         (std::pow(2 * pi * kz_ / Lz_, n) * zero_last_mode(kz_, kzmax(), n));
}
...
void FlowField::addPerturbation(int kx, int kz, Real mag, Real decay) {
  ...
  v *= pow(decay, k);
  w *= pow(decay, k);
  if (mx(kx) >= mxlocmin_ && mx(kx) < mxlocmin_ + Mxloc_ &&
      mz(kz) >= mzlocmin_ && mz(kz) < mzlocmin_ + Mzloc_) {
    int m_x = mx(kx);
    int m_z = mz(kz);
    for (int ny = nylocmin_; ny < nylocmax_; ++ny) {
  ...
  if (kz == 0 && kx != 0) {
    int m_x = mx(-kx);
    int m_z = mz(0); // -kz=0
    if (m_x >= mxlocmin_ && m_x < mxlocmin_ + Mxloc_ && mz(kz) >= mzlocmin_ &&
        mz(kz) < mzlocmin_ + Mzloc_) {
      for (int i = 0; i < Nd_; ++i)
        for (int ny = nylocmin_; ny < nylocmax_; ++ny) {
  ...
  if (Nd_ > 2) {
    ...
    for (int kx = Kxmin; kx <= Kxmax; ++kx)
      for (int kz = 0; kz <= Kzmax; ++kz) {
        ...
        Real norm =
            std::pow(decay, abs(2 * pi * kx / Lx_) + abs(2 * pi * kz / Lz_));
...
void FlowField::saveProfile(int mx, int mz, const string &filebase,
                            const ChebyTransform &trans) const {
  if (mpirank() > 0)
...
void FlowField::saveDivSpectrum(const string &filebase, bool kxorder,
  ...
  if (kxorder) {
    ...
    for (int mx = 0; mx < Mx(); ++mx) {
      Complex d_dx(0.0,
                   2 * pi * kx(mx) / Lx_ * zero_last_mode(kx(mx), kxmax(), 1));

      for (int mz = 0; mz < Mz(); ++mz) {
        Complex d_dz(0.0, 2 * pi * kz(mz) / Lz_ *
                              zero_last_mode(kz(mz), kzmax(), 1));
...
void FlowField::asciiSave(const string &filebase) const {
  ...
  os << "% Nx Ny Nz Nd == " << Nx_ << s << Ny_ << s << Nz_ << s << Nd_
     << " gridpoints\n";
  os << "% Mx My Mz Nd == " << Mx() << s << My() << s << Mz() << s << Nd_
     << " spectral modes\n";
  ...
  if (xzstate_ == Physical) {
    ...
    os << "%           os << Re(f.cmplx(mx, ny, mz, i) << ' ' << "
          "Im(f.cmplx(mx, ny, mz, i) << newline;\n";
  }
  if (xzstate_ == Physical) {
    for (int i = 0; i < Nd_; ++i)
    ...
    for (int i = 0; i < Nd_; ++i)
      for (long ny = 0; ny < Ny_; ++ny)
        for (int mx = 0; mx < Mx(); ++mx)
          for (int mz = 0; mz < Mz(); ++mz)
            os << setw(w) << Re(cmplx(mx, ny, mz, i)) << s << setw(w)
               << Im(cmplx(mx, ny, mz, i)) << nl;
  }
  os.close();
}

void FlowField::save(const string &filebase,
                     vector<string> component_names) const {
  string filename;
  ...
  if (ffsuffix)
    ...
    if (HAVE_HDF5)
      ...
      cferror("FlowField::save(filename) error : can't save to HDF5 file "
              "because HDF5 libraries are not installed. "
              "filename == " +
              filebase);
    }
  else if (ncsuffix)
    if (HAVE_NETCDF)
      ...
      cferror("FlowField::save(filename) error : can't save to NetCDF file "
              "because NetCDF libraries are not "
              "installed. filename == " +
              filebase);
    }
  else if (ascsuffix)
...
void FlowField::hdf5Save(const string &filebase) const {
  cferror("FlowField::hdf5save requires HDF5 libraries. Please install them "
          "and recompile channelflow.");
}
...
void FlowField::hdf5Save(const string &filebase) const {
  ...
  CfMPI_one = &CfMPI_single::getInstance();
#endif
  v = FlowField(Nx, Ny_, Nz, Nd_, Lx_, Lz_, a_, b_,
                CfMPI_one); // FlowField is only on process 0 -- serial io
  v.interpolate(u);
...
void FlowField::removePaddedModes(Real *rdata_io, lint Nxloc_io,
                                  lint nxlocmin_io, lint Mzloc_io,
                                  lint mzlocmin_io) const {
  /* Removal of padded modes in parallel and within the object (in contrast of
   * using interpolation onto a smaller grid, e.g. in hdf5Save)*/

  // bring flow field into intermediate state
  FlowField v(
      *this); // make copy to save back transformation from intermediate state
  if (xzstate_ == Physical)
    v.makeSpectral_xz();
  if (ystate_ == Spectral)
    v.makePhysical_y();

    // xy-transform of v=copy(this object) to intermediate state which is
    // xzstate=Spectral and ystate=Physical. The final data layout is z* x y* i
    // (* indicate distributed vars)
#ifdef HAVE_MPI
  if (nproc0() > 1) {
    for (lint iz = 0; iz < Mzloc_; iz++) {
      lint offset = 2 * iz * Mxloc_ * Nypad_ * Nd_;
      fftw_mpi_execute_r2r(v.t_iplan_.get(), &v.rdata_[offset],
                           &v.rdata_[offset]);
    }
  }
#endif
  ///////////////////////////////////////////////////////////////// some parts
  ///of block below could be members
  ...
  lint lzero = 0;
  lint Mzloc_trunc =
      min(Mzloc_, max(Mz_io - mzlocmin_,
                      lzero)); // local z-distribution after truncation
  ...
  lint Mx_chunk =
      Mx_io + np - (Mx_io % np); // Must be a multiple of np for Nloc_chunk to
  ...
  Nloc_chunk = 2 * fftw_mpi_local_size_many_transposed(
                       2, rank_chunk, howmany_io, FFTW_MPI_DEFAULT_BLOCK,
  ...
  for (int i = 0; i < Mzloc_trunc; i++)
    cdata_send.emplace_back(
        static_cast<Complex *>(fftw_malloc(Nloc_chunk * sizeof(Real))),
        fftw_free);
  for (int i = 0; i < Mzloc_io; i++)
    cdata_receive.emplace_back(
        static_cast<Complex *>(fftw_malloc(Nloc_chunk * sizeof(Real))),
        fftw_free);
  ...
  xz_iplan_io.reset(fftw_mpi_plan_many_dft_c2r(
      2, rank_1_io, howmany_io, FFTW_MPI_DEFAULT_BLOCK, FFTW_MPI_DEFAULT_BLOCK,
      fcdata_io, rdata_io, cfmpi_->comm1,
  ...
  xz_iplan_io.reset(fftw_plan_many_dft_c2r(
      rk, real_n, howmany, fcdata_io, cplx_embed, cplx_stride, cplx_dist,
  ...
  for (int mz = mzlocmin_; mz < mzlocmin_ + Mzloc_trunc; mz++)
    for (int mx = 0; mx < Mx_io / 2 + 1; mx++)
      for (int ny = nylocmin_; ny < nylocmin_ + Nyloc_; ny++)
        for (int i = 0; i < Nd_; i++)
          cdata_send[mz - mzlocmin_]
              .get()[i + Nd_ * ((ny - nylocmin_) + Nylocpad_ * mx)] =
              v.cdata_[i + Nd_ * ((ny - nylocmin_) +
                                  Nylocpad_ * (mx + Mx_ * (mz - mzlocmin_)))];
      ...
      for (int ny = nylocmin_; ny < nylocmin_ + Nyloc_; ny++)
        for (int i = 0; i < Nd_; i++)
          cdata_send[mz - mzlocmin_]
              ...
              v.cdata_[i +
                       Nd_ * ((ny - nylocmin_) +
                              Nylocpad_ * (mx_cut + Mx_ * (mz - mzlocmin_)))];
    }
#else
  for (int mz = mzlocmin_; mz < mzlocmin_ + Mzloc_trunc; mz++)
    for (int mx = 0; mx < Mx_io / 2 + 1; mx++)
  ...
  for (int mz = mzlocmin_io; mz < mzlocmin_io + Mzloc_io; mz++) {
    ...
    if ((rank == receiver) && (sender != receiver)) {
      ...
      MPI_Recv(reinterpret_cast<double *>(cdata_receive[pos].get()), Nloc_chunk,
               MPI_DOUBLE, sender, mz, *comm_world(), &mpistatus);
#else
...
#ifdef HAVE_MPI
        // non-blocking comm
        MPI_Ssend(reinterpret_cast<double *>(cdata_send[oldpos].get()),
                  Nloc_chunk, MPI_DOUBLE, receiver, mz, *comm_world());
#else
        cferror("Trying to use MPI without HAVE_MPI");
#endif
    } else {
      std::swap(cdata_receive[newpos],
                cdata_send[oldpos]); // shift data from old to new position
    }
  }
  ...
  for (int mz = mzlocmin_io; mz < mzlocmin_io + Mzloc_io; mz++)
    for (int mx = 0; mx < Mx_io; mx++)
      for (int ny = nylocmin_; ny < nylocmin_ + Nyloc_; ny++)
        for (int i = 0; i < Nd_; i++)
          cdata_io[i + Nd_ * ((ny - nylocmin_) +
                              Nylocpad_ * (mx + Mx_io * (mz - mzlocmin_io)))] =
              cdata_receive[mz - mzlocmin_io]
                  .get()[i + Nd_ * ((ny - nylocmin_) + Nylocpad_ * mx)];
#else
  for (int mz = mzlocmin_io; mz < mzlocmin_io + Mzloc_io; mz++)
    for (int mx = 0; mx < Mx_io; mx++)
  ...
  // final transformation which fills rdata_io with the physical field of the
  // selected Fourier modes
  fftw_execute(xz_iplan_io.get());
}

void FlowField::addPaddedModes(Real *rdata_io, lint Nxloc_io, lint nxlocmin_io,
                               lint Mzloc_io, lint mzlocmin_io) {
  // define data size parameters and allocate memory
  Complex *cdata_io = (Complex *)rdata_io;
  fftw_complex *fcdata_io = (fftw_complex *)cdata_io;
...
#ifdef HAVE_MPI
  lint rank_1_io[2] = {Nx_io_, Nz_io_};
  xz_plan_io.reset(fftw_mpi_plan_many_dft_r2c(
      2, rank_1_io, howmany_io, FFTW_MPI_DEFAULT_BLOCK, FFTW_MPI_DEFAULT_BLOCK,
      rdata_io, fcdata_io, cfmpi_->comm1,
  ...
  const int cplx_stride = 1;
  const int cplx_dist = Nx_io_ * Nzpad2_io;
  xz_plan_io.reset(fftw_plan_many_dft_r2c(
      rk, real_n, howmany, rdata_io, real_embed, real_stride, real_dist,
      fcdata_io, cplx_embed, cplx_stride, cplx_dist, fftw_flags_io));
#endif

  fftw_execute(xz_plan_io.get()); // make xz spectral without xy-transpose which
                                  // is done after MPI communication
...
#ifdef HAVE_MPI
      // blocking comm
      MPI_Ssend(reinterpret_cast<double *>(cdata_send[pos].get()), Nloc_chunk,
                MPI_DOUBLE, receiver, mz, *comm_world());
#else
      cferror("Trying to use MPI without HAVE_MPI");
#endif
    }
...
#ifdef HAVE_MPI
        MPI_Status mpistatus;
        MPI_Recv(reinterpret_cast<double *>(cdata_receive[newpos].get()),
                 Nloc_chunk, MPI_DOUBLE, sender, mz, *comm_world(), &mpistatus);
#else
        cferror("Trying to use MPI without HAVE_MPI");
#endif
      }
  ...
  for (int mz = mzlocmin_; mz < mzlocmin_ + Mzloc_trunc; mz++)
    for (int mx = 0; mx < Mx_io / 2 + 1; mx++)
      for (int ny = nylocmin_; ny < nylocmin_ + Nyloc_; ny++)
        for (int i = 0; i < Nd_; i++)
          cdata_[i + Nd_ * ((ny - nylocmin_) +
                            Nylocpad_ * (mx + Mx_ * (mz - mzlocmin_)))] =
              cdata_receive[mz - mzlocmin_]
                  .get()[i + Nd_ * ((ny - nylocmin_) + Nylocpad_ * mx)];
  ...
  for (int mz = mzlocmin_; mz < mzlocmin_ + Mzloc_trunc; mz++) {
    for (int mx = Mx_io / 2 + 1; mx < Mx_ - Mx_io / 2 + 1; mx++) {
      ...
      for (int ny = nylocmin_; ny < nylocmin_ + Nyloc_; ny++)
        for (int i = 0; i < Nd_; i++)
          cdata_[i + Nd_ * ((ny - nylocmin_) +
                            Nylocpad_ * (mx + Mx_ * (mz - mzlocmin_)))] = zero;
    }
  }
...
#if HAVE_NETCDF
void FlowField::writeNetCDF(const string &filebase,
                            vector<string> component_names) const {
  ...
  if (not(padded_) && (xzstate_ == Spectral || ystate_ == Spectral)) {
    // the copy for changing the state in the case padded_=true is done in
    // method "removePaddedModes()"
    FlowField v(*this);
    v.makePhysical();
    ...
    if (not(padded_)) {
      ...
      Nloc_io = 2 * fftw_mpi_local_size_many_transposed(
                        2, rank_0_io, howmany_io, FFTW_MPI_DEFAULT_BLOCK,
                        FFTW_MPI_DEFAULT_BLOCK, cfmpi_->comm1, &Nxloc_io,
                        &nxlocmin_io, &Mzloc_io, &mzlocmin_io);
#endif

      rdata_io_handle.reset(
          static_cast<Real *>(fftw_malloc(Nloc_io * sizeof(Real))));
    ...
    if ((status = nc_create_par(nc_name.c_str(), NC_NETCDF4 | NC_MPIIO,
                                *comm_world(), info, &ncid)))
      ERR(status);
    doesIO = true;
#else
    if (taskid() == 0) {
    ...
    if (doesIO) { // header is handled either by master (without parNC) or by
                  // all tasks collectively (with parNC)
      ...
      if ((status = nc_put_att_text(ncid, NC_GLOBAL, "Conventions",
                                    strlen(cf_conv), cf_conv)))
        ERR(status);
      if ((status =
               nc_put_att_text(ncid, NC_GLOBAL, "title", strlen(title), title)))
        ERR(status);
      if ((status = nc_put_att_text(ncid, NC_GLOBAL, "format_version",
                                    fversion.size(), fversion.c_str())))
        ERR(status);
      if ((status = nc_put_att_text(ncid, NC_GLOBAL, "channelflow_version",
                                    strlen(CHANNELFLOW_VERSION),
                                    CHANNELFLOW_VERSION)))
        ERR(status);
      if ((status =
               nc_put_att_text(ncid, NC_GLOBAL, "compiler_version",
                               strlen(COMPILER_VERSION), COMPILER_VERSION)))
        ERR(status);
      if ((status = nc_put_att_text(ncid, NC_GLOBAL, "git_revision",
                                    strlen(g_GIT_SHA1), g_GIT_SHA1)))
        ERR(status);
      if ((status = nc_put_att_text(ncid, NC_GLOBAL, "time", strlen(tbuffer),
                                    tbuffer)))
        ERR(status);
      if ((status = nc_put_att_text(ncid, NC_GLOBAL, "host_name",
                                    strlen(hostname), hostname)))
        ERR(status);
      if ((status = nc_put_att_text(ncid, NC_GLOBAL, "references",
      ...
      if ((status = nc_put_att_int(ncid, NC_GLOBAL, "Nz", NC_INT, 1, &Nz_)))
        ERR(status);
      if ((status =
               nc_put_att_double(ncid, NC_GLOBAL, "Lx", NC_DOUBLE, 1, &Lx_)))
        ERR(status);
      if ((status =
               nc_put_att_double(ncid, NC_GLOBAL, "Lz", NC_DOUBLE, 1, &Lz_)))
        ERR(status);
      ...
      /* Define the dimensions and the grid variables (chosen here to have the
       * same name) */
      for (int i = 0; i < nd; i++) {
        if ((status = nc_def_dim(ncid, dim_name[i].c_str(), dim_size[i],
                                 &dimid[nd - i - 1])))
          ERR(status);
        int dimid1[1];
        dimid1[0] = dimid[nd - i - 1];
        if ((status = nc_def_var(ncid, dim_name[i].c_str(), NC_DOUBLE, 1,
                                 dimid1, &gridid[i])))
          ERR(status);
      }

      /* Define the data variables. The type of the variable in this case is
       * NC_DOUBLE (NC variable type = 6). */
      for (int i = 0; i < Nd_; i++)
        if ((status = nc_def_var(ncid, var_name[i].c_str(), NC_DOUBLE, nd,
                                 dimid.data(), &varid[i])))
          ERR(status);
    ...
    if ((status = nc_put_vara_double(ncid, gridid[0], start_grid, count_grid,
                                     &xgrid[0])))
      ERR(status);
    start_grid[0] = start_var[1];
    count_grid[0] = count_var[1];
    if ((status = nc_put_vara_double(ncid, gridid[1], start_grid, count_grid,
                                     &ygrid[0])))
      ERR(status);
    start_grid[0] = start_var[0];
    count_grid[0] = count_var[0];
    if ((status = nc_put_vara_double(ncid, gridid[2], start_grid, count_grid,
                                     &zgrid[0])))
      ERR(status);
#else
    if (doesIO) {
    ...
    for (int i = 0; i < Nd_; i++) {
      ...
      for (int ny = 0; ny < Ny_; ny++)
        for (int nx = 0; nx < Nx_io; nx++)
          for (int nz = 0; nz < Nz_io; nz++)
            ...
            var[nx + Nx_io * (ny + Ny_ * nz)] =
                rdata_io[nz + Nzpad_io * (nx + Nx_io * (ny + Ny_ * i))];

      /* Write NetCDF variable */
      if ((status = nc_put_vara_double(ncid, varid[i], start_var, count_var,
                                       var.data())))
        ERR(status);
      ...
      for (int ny = nylocmin_; ny < nylocmin_ + Nyloc_; ny++)
        for (int nz = 0; nz < Nz_io; nz++)
          for (int nx = nxlocmin_io; nx < nxlocmin_io + Nxloc_io; nx++)
            ...
            var[(nx - nxlocmin_io) +
                Nxloc_io * ((ny - nylocmin_) + Nyloc_ * nz)] =
                rdata_io[i + Nd_ * ((ny - nylocmin_) +
                                    Nylocpad_ *
                                        (nz + Nzpad_io * (nx - nxlocmin_io)))];
#if HAVE_NETCDF_PAR
      ...
      if ((status = nc_put_vara_double(ncid, varid[i], start_var, count_var,
                                       var.data())))
        ERR(status);
#else /* block below defines communication in the case of using mpi without    \
         parallel NetCDF */
      ////////////////////////////////////////////////////////////
      ...
      if (taskid() == 0) {
        ...
        for (int nx = 0; nx < Nxloc_io; nx++)
          for (int ny = 0; ny < Nyloc_; ny++)
            for (int nz = 0; nz < Nz_io; nz++)
              mastervar[nx + Nx_io * (ny + Ny_ * nz)] =
                  var[nx + Nxloc_io * (ny + Nyloc_ * nz)];
        MPI_Status mpistatus;
        // mpi communication: sending location parameters is saver than
        // calculating them on master
        ...
        for (int rk = 1; rk < np; rk++) {
          ...
          MPI_Recv(&ystart, 1, MPI_INT, rk, rk + np * 2, *comm_world(),
                   &mpistatus);
          MPI_Recv(&ysize, 1, MPI_INT, rk, rk + np * 3, *comm_world(),
                   &mpistatus);
          int rec_varsize = Nz_io * ysize * xsize;
          if (rec_varsize > 0) {
            // maybe could use var, but local data is not of equal size amongst
            // procs
            auto rec_var = vector<double>(rec_varsize, 0.0);
            MPI_Recv(rec_var.data(), rec_varsize, MPI_DOUBLE, rk, rk + 2 * np,
                     *comm_world(), &mpistatus);
            for (int nx = 0; nx < xsize; nx++)
              for (int ny = 0; ny < ysize; ny++)
                for (int nz = 0; nz < Nz_io; nz++)
                  mastervar[(nx + xstart) +
                            Nx_io * ((ny + ystart) + Ny_ * nz)] =
        ...
        if ((status = nc_put_vara_double(ncid, varid[i], masterstart,
                                         mastercount, mastervar.data())))
          ERR(status);
      } else {
        int tmp1 = nylocmin_;
        int tmp2 = Nyloc_;

        // FIXME: Here we are sending a value of type lint (a.k.a ptrdiff_t
        // a.k.a long int on most archs)
        // FIXME: using a MPI_INT instead of an MPI long. The matching receive
        // should be probably adjusted, but for
        ...
        if (varsize > 0) {
          MPI_Ssend(var.data(), varsize, MPI_DOUBLE, 0, taskid() + 2 * np,
                    *comm_world()); // blocking comm
        }
      }
      ////////////////////////////////////////////////////////////
#endif
...
void FlowField::writeNetCDF(const string &filebase,
                            vector<string> component_names) const {
  cferror("FlowField::writeNetCDF requires NetCDF libraries. Please install "
          "them and recompile channelflow.");
}
#endif
...
void FlowField::readNetCDF(const string &filebase) {
  ...
  MPI_Info info = MPI_INFO_NULL;
  if ((status =
           nc_open_par(nc_name.c_str(), NC_MPIIO, *comm_world(), info, &ncid)))
    ERR(status);
#else
  int np = nproc0() * nproc1();
  for (int rk = 0; rk < np; rk++) { // enforcing ordered opening of file reduces
                                    // the risk of dead lock
    if (taskid() == rk) {
      // all proc need to open because all will read in their part (does not
      // work with sending "ncid")
  ...
  /* General inquiry of how many netCDF variables, dimensions, and global
  ...
  if (taskid() == 0) { // header is handled by master node
    ...
    Nd_ =
        nvars_in -
        ndims_in; // the grid along each dimension is also counted as variables

    /* Read necessary global attributes from file */
    char attname[NC_MAX_NAME + 1];
    for (int attid = 0; attid < ngatts_in;
         attid++) { // loop over all global attributes
  ...
  if ((Nx_io == Nx_) && (Ny_io == Ny_) &&
      (Nz_io == Nz_)) { // IO with padded modes
    padded_ = false;
    Nxloc_io = Nxloc_;
    nxlocmin_io = nxlocmin_;
    Mzloc_io = mzlocmin_io =
        0; // variables not initialized, should not be used when padded_==false
    Nzpad_io = Nzpad_;
    rdata_io = rdata_;
  } else if ((Nx_io == Nx_io_) && (Ny_io == Ny_) &&
             (Nz_io == Nz_io_)) { // IO without padded modes
    padded_ = true;
    // allocate the memory for the data without padded modes
    lint Mz_io = Nz_io_ / 2 + 1;
    Nxloc_io = Nx_io_;
    ...
    // Avoids "variable uninitialized" warning. When file is corrupted, results
    // should not be used.
  ...
  for (int varid = 0; varid < nvars_in; varid++) {
    ...
    if ((strcmp(varname, "X") != 0) && (strcmp(varname, "Y") != 0) &&
      ...
      if (varsize > 0) {
        ...
        start[0] = 0;
        count[0] = 1; // because they do not read in data. This happens for
                      // certain combinations
        start[1] = 0;
        count[1] = 1; // of grid size to job distributions, e.g. np0>Ny.
...
#endif               // HAVE_NETCDF_PAR
#endif               // HAVE_MPI
      if ((status = nc_get_vara_double(ncid, varid, start.data(), count.data(),
                                       var.data())))
        ERR(status);
      ...
      for (int nx = nxlocmin_io; nx < nxlocmin_io + Nxloc_io; nx++)
        for (int ny = nylocmin_; ny < nylocmin_ + Nyloc_; ny++)
          for (int nz = 0; nz < Nz_io; nz++)
            rdata_io[ivar +
                     Nd_ * ((ny - nylocmin_) +
                            Nylocpad_ * (nz + Nzpad_io * (nx - nxlocmin_io)))] =
                var[(nx - nxlocmin_io) +
                    Nxloc_io * ((ny - nylocmin_) + Nyloc_ * nz)];
#else
      for (int nx = 0; nx < Nx_io; nx++)
...
#else
void FlowField::readNetCDF(const string &filebase) {
  cferror("FlowField::readNetCDF requires NetCDF libraries. Please install "
          "them and recompile channelflow.");
}
#endif // HAVE_NETCDF
...
FlowField quadraticInterpolate(cfarray<FlowField> &un, const cfarray<Real> &mun,
                               Real mu, Real eps) {
  // Extrapolate indpt param R and box params as function of mu

  if (un.length() != 3 || mun.length() != 3) {
    stringstream serr;
    serr << "error in quadraticInterpolate(cfarray<FlowField>& un, "
            "cfarray<Real>& mun, Real mu, Real eps)\n";
  ...
  if (un[1].Nx() != Nx || un[2].Nx() != Nx || un[1].Ny() != Ny ||
      un[2].Ny() != Ny || un[1].Nz() != Nz || un[2].Nz() != Nz ||
      un[1].Nd() != Nd || un[2].Nd() != Nd) {
    cferror("error in quadraticInterpolate(cfarray<FlowField>& un, "
            "cfarray<Real>& mun, Real mu, Real "
    ...
    for (int ny = nylocmin; ny < nylocmax; ++ny)
      for (int nx = nxlocmin; nx < nxlocmax; ++nx)
        for (int nz = 0; nz < Nz; ++nz) {
          ...
          u(nx, ny, nz, i) =
              isconst(fn, eps) ? fn[0] : quadraticInterpolate(fn, mun, mu);
        }
...
FlowField polynomialInterpolate(cfarray<FlowField> &un, cfarray<Real> &mun,
                                Real mu) {
  // Extrapolate indpt param R and box params as function of mu
...
void hdf5write(int i, const string &name, H5::H5File &h5file) {
  herr_t status =
      H5Tconvert(H5T_NATIVE_INT, H5T_STD_I32BE, 1, &i, NULL, H5P_DEFAULT);
  if (status < 0) {
  ...
  H5::Attribute attr =
      rootgr.createAttribute(name.c_str(), H5::PredType::STD_I32BE, dspace);
  attr.write(H5::PredType::STD_I32BE, &i);
}

void hdf5write(Real x, const string &name, H5::H5File &h5file) {
  herr_t status =
      H5Tconvert(H5T_NATIVE_DOUBLE, H5T_IEEE_F64BE, 1, &x, NULL, H5P_DEFAULT);
  if (status < 0) {
    cout << "HDF5 datatype conversion failed! Status: " << status;
  }
  ...
  H5::DataSpace dspace = H5::DataSpace(H5S_SCALAR);
  H5::Attribute attr =
      rootgr.createAttribute(name.c_str(), H5::PredType::IEEE_F64BE, dspace);
...
void hdf5write(const Vector &v, const string &name, H5::H5File &h5file) {
  ...
  H5::DataSet dset =
      h5file.createDataSet(name.c_str(), H5::PredType::IEEE_F64BE, dspace);
  herr_t status = H5Tconvert(H5T_NATIVE_DOUBLE, H5T_IEEE_F64BE, N, data.data(),
                             NULL, H5P_DEFAULT);
...
void hdf5write(const FlowField &u, const string &name, H5::H5File &h5file) {
  ...
  auto data =
      vector<double>(dsize, 0.0); // create cfarray of data to be written
  ...
  const hsize_t dimsf[4] = {(hsize_t)Nd, (hsize_t)Nz, (hsize_t)Ny,
                            (hsize_t)Nx}; // dataset dimensions

  H5::DataSpace dspace(4, dimsf); // create dataspace
  H5::DataSet dset =
      h5file.createDataSet(name.c_str(), H5::PredType::IEEE_F64BE, dspace);
  herr_t status = H5Tconvert(H5T_NATIVE_DOUBLE, H5T_IEEE_F64BE, data.size(),
                             data.data(), NULL, H5P_DEFAULT);
  if (status < 0) {
    cout << "HDF5 datatype conversion failed! Status: " << status;
  }
...
void hdf5read(FlowField &u, const string &name, H5::H5File &h5file) {
  ...
  if (u.taskid() == 0) {
    ...
    H5Tconvert(H5T_IEEE_F64BE, H5T_NATIVE_DOUBLE, buff.size(), buff.data(),
               NULL, H5P_DEFAULT);
...
void hdf5addstuff(const std::string &filebase, Real nu, ChebyCoeff Ubase,
                  ChebyCoeff Wbase) {
  H5std_string h5name = appendSuffix(filebase, ".h5");
  H5::H5File h5file(h5name, H5F_ACC_TRUNC);
...
int field2vector_size(const FlowField &u) {
  ...
  // but I've not been able to twist my head enough to adapt it do distributed
  // FlowFields. Since it doesn't take that much time, we now perform the loops
  // twice, once empty to determine cfarray sizes and once with the actual data
  // copying. // Tobias
  int N = 0;
...
void field2vector(const FlowField &u, VectorXd &a) {
  ...
  // The implementation minimizes the necessary communication operations
  // (acutally 0 in field2vector and Kz when restoring in vector2field) but
  // results in an unequal number of vector elements per process. The other
  // option - the same number of vector elements per process - would require a
  // lot of communication, which is way less time-efficient in most situations.
  ...
  for (int kx = 1; kx <= Kx; ++kx) {
    ...
    if (u.taskid() == u.task_coeff(mx, 0)) {
      ...
      for (int ny = 3; ny < Ny - 1;
           ++ny) { // u BCs => 0,1; v BC => 2; div => Ny-1
...
void vector2field(const VectorXd &a, FlowField &u) {
  ...
  for (int kx = 1; kx <= Kx; ++kx) {
    ...
    for (int ny = 0; ny < Ny; ++ny) {
      if (u.taskid() == send_id &&
          send_id == rec_id) { // all is on the same process -> just copy
        ...
        if (u.taskid() == rec_id) {
          ...
          MPI_Status status;
          MPI_Recv(&tmp0, 1, MPI_DOUBLE_COMPLEX, send_id, 0, MPI_COMM_WORLD,
                   &status);
          MPI_Recv(&tmp1, 1, MPI_DOUBLE_COMPLEX, send_id, 1, MPI_COMM_WORLD,
                   &status);
          MPI_Recv(&tmp2, 1, MPI_DOUBLE_COMPLEX, send_id, 2, MPI_COMM_WORLD,
                   &status);
